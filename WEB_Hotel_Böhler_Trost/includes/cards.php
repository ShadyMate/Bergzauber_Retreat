<div class="card-group">
    <div class="card">
        <img class="card-img-top" src="../res/img/hotelzimmer.jpg" alt="Hotelzimmer">
        <div class="card-body text-center">
            <h5 class="card-title">Zimmerangebot</h5>
            <p class="card-text">Hier finden Sie alle Angebote, sowie Informationen zur Buchung</p>
            <a href="../php/Zimmer.php" class="btn btn-primary">Zu den Zimmern</a>
        </div>
    </div>
    <div class="card">
        <img class="card-img-top" src="../res/img/radfahren.jpg" alt="Radweg">
        <div class="card-body text-center">
            <h5 class="card-title">Aktivitäten</h5>
            <p class="card-text">Aktivitäten in der Umgebung</p>
            <a href="../php/aktivitaeten.php" class="btn btn-primary">Zu den Aktivitäten</a>
        </div>
    </div>
    <div class="card">
        <img class="card-img-top" src="../res/img/Spaimagerezied.jpeg" alt="Spa">
        <div class="card-body text-center">
            <h5 class="card-title">Unser Spa Angebot</h5>
            <p class="card-text">Wir haben eine große Auswahl an Spa Angeboten für sie.</p>
            <a href="../php/spa.php" class="btn btn-primary">Zum Spa Angebot</a>
        </div>
    </div>
</div>